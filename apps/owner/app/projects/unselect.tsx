...
import type { Route } from "./+types/unselect";
import { getAccessToken } from "~/auth.server";

export async function action({ request, params: { id, proposalId } }: Route.ActionArgs) {
  const token = await getAccessToken(request);

  const response = await fetch(new URL(`/api/v1/orderer/rfps/${id}/proposals/${proposalId}/unselect`, process.env.BACKEND_API_URL), {
    method: 'POST',
    headers: {
